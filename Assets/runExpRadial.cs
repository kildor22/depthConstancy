...
public class runExpRadial : MonoBehaviour
{
    /// <summary>
    ...
    void InstantiateStimuli()
    {
        ...
        // Do the calculation for the angle of rotation here

        // refObj.transform.position.x
        float ang =
            Mathf.Asin((stimX - refObj.transform.position.x) / 0.8f) * 2.0f;
        stimObj = (GameObject)Instantiate(mdl,
           new Vector3(stimX, stimY, 500.8f), Quaternion.identity);
        Debug.Log(ang*(180.0f/Mathf.PI));
        //stimObj.transform.rotation = Quaternion.Euler(0, ang * (180.0f / Mathf.PI), 0);
        stimObj.transform.Rotate(0.0f, ang * (180.0f / Mathf.PI), 0.0f);
    }
